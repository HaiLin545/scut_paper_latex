...
无模板方法不依赖底层参数化人体模型，直接从图像中推断穿衣人体的完整三维几何。PIFu\cite{saito2019pifu}提出像素对齐隐式函数，将二维图像特征与三维坐标深度融合，实现单图高保真重建，图\ref{fig:pifu}展示了PIFu的网络结构。PIFuHD\cite{saito2020pifuhd}进一步融合高分辨率的法向预测网络，在褶皱、衣纹等细节上实现更高精度的重建。ICON\cite{xiu2022icon}结合预测的表面法线信息对隐式曲面进行局部优化，增强了对遮挡区域和复杂姿态的鲁棒性。ECON\cite{xiu2023econ}进一步融合法线预测与隐式重建，实现了更完整的穿衣人体几何修复。然而，上述方法均将人体与衣物作为不可分割的整体重建，缺乏对服装语义的独立建模，因此难以支持换装、物理仿真等下游应用。
